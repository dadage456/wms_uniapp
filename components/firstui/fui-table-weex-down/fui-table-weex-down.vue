<template>
	<!--本文件由FirstUI授权予赵*河（会员ID：   2928）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-table__weex-wrap">
		<scroll-view
			fixFreezing="true"
			class="fui-table__nvue-wrap"
			:class="{ 'fui-swiper-page': fixFreezing }"
			scroll-x="true"
			...
			>
				<header :style="{ width: totalW + 'px' }">
					<view
						...
						:style="{
							width: totalW + 'px',
							borderBottomColor: horBorder ? borderColor : 'transparent',
							borderTopColor: horBorder && show ? borderColor : 'transparent'
						}"
						v-if="show"
					>
						<view
							class="fui-table--td"
							:class="{
								'fui-table__border-right': border,
								'fui-table__border-left': border && index === 0,
								'fui-table__center': (item.align || align) === 'center',
								'fui-table__right': (item.align || align) === 'right'
							}"
							:style="{
								borderRightColor: border ? borderColor : 'transparent',
								borderLeftColor: border && index === 0 ? borderColor : 'transparent',
								background: item.background || headerBgColor,
								width: item.width + divideW + 'px',
								paddingTop: padding + 'rpx',
								paddingBottom: padding + 'rpx'
							}"
							v-for="(item, index) in hData"
							:key="index"
							@tap.stop="tableSort(index, false)"
						>
							<view
								class="fui-table__checkbox"
								...
								@tap.stop="selectionAll"
							>
								<view class="fui-table__checkmark" :style="{ borderBottomColor: checkmarkColor, borderRightColor: checkmarkColor }"></view>
							</view>
							<text
								v-else
								class="fui-table--td-text"
								:class="{
									'fui-text__center': (item.align || align) === 'center',
									'fui-text__right': (item.align || align) === 'right',
									'fui-td__ellipsis': ellipsis
								}"
								:style="{ width: item.width + divideW + 'px', color: item.color || color, fontSize: (item.size || size) + 'rpx', fontWeight: fontWeight }"
							>
								{{ item.label || item.prop }}
							</text>
							<view class="fui-table__sort-icon" :style="{ right: (item.sortRight || 40) + 'rpx' }" v-if="item.sortable">
								<fui-icon
									:name="item.sort === 'descending' ? 'turningdown' : 'turningup'"
									:size="item.sortSize || 28"
									:color="item.sort ? item.sortColor || '#333' : item.color || color"
								></fui-icon>
							</view>
							<view class="fui-table__td-sk" :style="{ backgroundColor: borderColor }" v-if="border && item.fixed === 'right'"></view>
						</view>
					</view>
				</header>

				<cell :style="{ width: totalW + 'px' }" v-for="(item, index) in tableData" :key="item['data1s']">
					<view
						class="fui-table--tr"
						:class="{ 'fui-table__border-bottom': horBorder, 'fui-table__border-top': horBorder && !show && index === 0 }"
						:key="item['outtaskitemid']"
						:style="{
							width: totalW + 'px',
							borderBottomColor: horBorder ? borderColor : 'transparent',
							borderTopColor: horBorder && !show && index === 0 ? borderColor : 'transparent'
						}"
					...
					>
						<view
							...
							:class="{
								'fui-table__border-right': border,
								'fui-table__border-left': border && idx === 0,
								'fui-table__center': (model.align || align) === 'center',
								'fui-table__right': (model.align || align) === 'right',
								'fui-table__td-wrap': model.type === 3
							}"
							v-for="(model, idx) in hData"
							:key="model.tdId"
							:style="{
								borderRightColor: border ? borderColor : 'transparent',
								borderLeftColor: border && idx === 0 ? borderColor : 'transparent',
								background: highlight && isHighlight(index) ? '#238E23' : item.background || ((index + 1) % 2 === 0 && stripe ? stripeColor : background),
								width: model.width + divideW + 'px',
								paddingTop: padding + 'rpx',
								paddingBottom: padding + 'rpx'
							}"
							@tap.stop="rowClick(index)"
						>
							<template v-if="model.type !== 3">
								<text
									class="fui-table--td-text"
									:class="{
										'fui-text__center': (model.align || align) === 'center',
										'fui-text__right': (model.align || align) === 'right',
										'fui-td__ellipsis': ellipsis
									}"
									:style="{
										color: getColColor(model, item[model.prop], index, idx),
										fontSize: (model.textSize || textSize) + 'rpx',
										width: model.width + divideW + 'px'
									}"
									v-if="idx == 0 && isTotal && index == tableData.length - 1"
								>
									{{ totalText }}
								</text>
								...
								<view
									class="fui-table__checkbox"
									:class="{
										'fui-table__checkbox-color': (!checkboxColor || checkboxColor === true) && item.is_selected,
										'fui-table__disabled': item.is_disabled
									}"
									:style="{
										background: item.is_selected ? getCheckboxColor : 'transparent',
										borderColor: item.is_selected ? getCheckboxColor : checkboxBorderColor
									}"
									v-else-if="model.type === 'selection'"
									@tap.stop="selectionChange(index)"
								>
									<!-- 									<view class="fui-table__checkmark" :style="{ borderBottomColor: checkmarkColor, borderRightColor: checkmarkColor }"></view>
 -->
									<view
										class="fui-table__checkmark"
										:style="{
											borderBottomColor: highlight && isHighlight(index) ? '#238E23' : checkmarkColor,
											borderRightColor: highlight && isHighlight(index) ? '#238E23' : checkmarkColor
										}"
									></view>
								</view>
								<image
									class="fui-table--td-img"
									:src="item[model.prop] || ''"
									mode="widthFix"
								...
								<text
									...
									v-else-if="model.type === 4"
									:style="{
										color: model.color || getColColor(model, item[model.prop], index, idx),
										fontSize: (model.textSize || textSize) + 'rpx',
										width: model.width + divideW + 'px'
									}"
								>
								...
								>
									{{ item[model.prop] == 0 ? item[model.prop] : item[model.prop] || '' }}
								</text>
							...
							<template v-else>
								<text v-if="isTotal && index == tableData.length - 1" :style="{ fontSize: textSize + 'rpx', fontWeight: 'normal' }">-</text>
								<view class="fui-table--tdinner" v-else>
									...
									<fui-button
										v-for="(btn, j) in model.buttons"
										:key="btn.bId"
										:type="btn.type"
										size="10"
										btn-size="mini"
										style="margin-right: 8rpx"
										@tap.stop="handleTap(index, j)"
									>
										{{ btn.text }}
									</fui-button>
								</view>
							</template>
							<view class="fui-table__td-sk" :style="{ backgroundColor: borderColor }" v-if="border && model.fixed === 'right'"></view>
						</view>
					</view>
				</cell>

				<cell :style="{ width: width + 'px' }" v-if="itemList.length === 0 && emptyText !== true && emptyText !== ''">
					<view class="fui-table--empty" :style="{ width: width + 'px' }" :class="{ 'fui-table__empty-ab': totalW > width && height > 0 && height != 0 }">
		...
		<template v-if="isDrag">
			<view
				:ref="`ref_move_${index}`"
				class="fui-table__td-move"
				v-for="(item, index) in hData"
				:key="index"
				:style="{ left: item.leftW - scrollx + 'px', background: index == moveIndex ? activeLineColor : 'transparent' }"
				@longpress="tdlongtap(index, $event)"
...
export default {
	...
	methods: {
		...
		handleHeader(header) {
			...
			this.hData = vals;
			console.log('hData数：' + this.hData.length);
		},
		...
		handleData(vals) {
			...
			if (this.isTotal) {
				...
				this.header.forEach((h) => {
					...
					const prop = h.prop;
					// 取消计算
					row[prop] = Number(this.getTotalNum(table, prop)) || '';
				});
				table.push(row);
			}

			this.tableData = table;
			console.log('table数：' + this.tableData.length);
