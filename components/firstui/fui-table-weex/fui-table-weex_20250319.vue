					...
					>
						<template v-if="model.type !== 3">
							...
							<text
								...
								v-else
								:style="{ color: getColColor(model, item[model.prop], index), fontSize: (model.textSize || textSize) + 'rpx', width: model.width + divideW + 'px'  }"
							>
								{{ item[model.prop] == 0 ? item[model.prop] : item[model.prop] || '' }}
							</text>
						</template>
						<template v-else>
							<fui-button v-for="(btn, j) in model.buttons" :key="btn.bId" :type="btn.type" size="10" btn-size="mini" style="margin-right: 8rpx" @tap.stop="handleTap(index, j)">
								{{ btn.text }}
							</fui-button>
